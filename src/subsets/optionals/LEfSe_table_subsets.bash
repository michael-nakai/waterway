...
if [ "$subset" = true ] && [ "$generate_lefse_tables" = true ]; then
    for repqza in ${qzaoutput}*/subsets/*/rep-seqs.qza
    do
        if [ "$sklearn_done" = true ]; then

            # Define general filepaths
            qzaoutput2=${repqza%"rep-seqs.qza"}
            echolog "Starting ${CYAN}LEfSe table autogeneration${NC} for $qzaoutput2"
            ...
            do
                talkative "Starting table generation for $tablefile"
    ...
    echolog "    ${GREEN}Finished LEfSe table generation for subsets${NC}\n"
fi