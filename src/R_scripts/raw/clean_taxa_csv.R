...
for (value in origTable[1,]) {
    if (grepl("k__", value, fixed = TRUE) && grepl(";", value, fixed = TRUE)) { # Check if value includes a k__ and a semicolon, which all taxa have
...
newTable <- newTable %>%
    select(which(colnames(newTable)=="sample_id"), everything()) # Do the moving of "sample_id" to column 1
newTable <- newTable %>%
    select(which(colnames(newTable)==group_to_compare), everything()) # Do the moving of group_to_compare to column 1
